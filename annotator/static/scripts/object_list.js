...
function RenderObjectList() {
  ...
  for(var i=0; i < Npolygons; i++) {
    ...
    if(is_currently_shown && (((ii<num_orig_anno)&&((view_Existing&&!isDeleted)||(isDeleted&&view_Deleted))) || ((ii>=num_orig_anno)&&(!isDeleted||(isDeleted&&view_Deleted))))) {
      ...
      html_str += '<div class="objectListLink" id="LinkAnchor' + ii + '" style="z-index:1; margin-right:2rem;margin-bottom: 0.3rem;display:inline-block;" ';
...
function ShowTesseractAnnotations(){
  ...
  var url = '/annotate/auto/tesseract?mode=' + main_media.file_info.mode + '&service=' + serviceURL + '&username=' + username + '&collection=' + collection_name + '&folder=' + main_media.file_info.dir_name + '&image=' + main_media.file_info.im_name;
  console.log("\n**************\n")
  console.log('object_list: \n')
  console.log(url)
  console.log("\n**************\n")

  document.getElementById("loading_anno_services").style.display = "block"
  // document.getElementById("click2").click()
  document.getElementById( 'loading_anno_services' ).scrollIntoView();
  var json_req = $.getJSON(url, function(result, status, xhr){
    console.log("Annotations recieved from tesseract");

    document.getElementById( 'stopPos1' ).scrollIntoView();
    document.getElementById("loading_anno_services").style.display = "none"
    ...
    // Write XML to server:
    console.log("tesseract submit url: ",SubmitXmlUrl)
    updateServiceURL = serviceURL.replace(/\//g, ",");
    console.log("updated tesseract submit url: ",updateServiceURL)
    WriteXML('/add/services/static/perl/submit.cgi/'+ updateServiceURL,LM_xml,function(){return;});
...
function ShowModelAnnotations(pool_name){
  ...
  // document.getElementById('submit_auto').style.cursor = "not-allowed"
  // document.getElementById('submit_auto').style.opacity = "1.0"
  var json_req = $.getJSON(url, function(result, status, xhr){
      console.log('step2')  
      document.getElementById('scroll_up_done').style.display = "inline-block"
    //  document.getElementById('submit_auto').style.cursor = "not-allowed"
      setTimeout(() => {
        document.getElementById('scroll_up_done').style.display = "none"
  ...
  console.log('I am not in main loop')
}
}
/**
