...
def create_model_directory(user):
    try:
        ...
        print(ex)
        return "-1" 
def neg_create_training_data_images(user,directory_name,a,b,document):
  dataset = []
  max_patches = 10
  label_tensor = []
  for x in range(max_patches):
    label_tensor.append('non-pattern')
  main_tensor = create_patches(max_patches,directory_name+"/data/"+document,b,a)
  dataset.append(numpy.array(main_tensor))
  label_tensor = numpy.array(label_tensor)
  dataset.append(numpy.array(label_tensor))
...
def neg_create_distinct_annotations(user,directory_name,a,b,document):
  return neg_create_training_data_images(user,directory_name,a,b,document), ['non-pattern']
    ...
    while i<height_of_image:
        ...
        while j<width_of_image:
            ...
            if(prob<=probability):
                ...
                list_of_other_characters = list(dataset.keys())
                list_of_other_characters.remove(character)
                number_of_other_characters = len(list_of_other_characters)
                random_character = list_of_other_characters[random.randint(0,number_of_other_characters-1)]
                other_images = dataset[random_character]
                number_of_other_images = other_images.shape[0]
                random_image_number = random.randint(0,number_of_other_images-1)
                sub_image = other_images[random_image_number]
...
def make_dataset(class_label,dataset,no_of_images=700,probability=0.6):
    my_x = []
    my_y = []
...
def train_model(train_loader1,train_loader2 = None,cuda = "cuda:0",kernel_height=30,kernel_width=30,max_epochs = 1,net=None):
    
    device = torch.device(cuda if torch.cuda.is_available() else "cpu")
    
    if(net == None):
      net = cnn(kernel_height,kernel_width).to(device)
    loss_fn = nn.MSELoss()
...
def lookup(annotation,user,document):
    ...
    if(directory_name != "-1"):
        ...
        training_dataset_b,distinct_names_b = neg_create_distinct_annotations(user,directory_name,response[1],response[2],document)
        data_dictionary = {}
        training_dataset_a[0] = 255-training_dataset_a[0]
        training_dataset_b[0] = 255-training_dataset_b[0]
        ...
        for x in range(len(training_dataset_b[1])):
          data_dictionary[training_dataset_b[1][x]] = []

        for x in range(len(training_dataset_a[1])):
          data_dictionary[training_dataset_a[1][x]].append(np.array(training_dataset_a[0][x]))
        for x in range(len(training_dataset_b[1])):
          data_dictionary[training_dataset_b[1][x]].append(np.array(training_dataset_b[0][x]))
        ...
        while(True):
          ...
          for x in training_dataset_b[0]:
            ims.append(np.array(x))
          lbls = []
          for x in training_dataset_a[1]:
            lbls.append(label_map[x])
          for x in training_dataset_b[1]:
            lbls.append(label_map[x])
          tensor = []
          ...
          best_model,loss = train_model(train_loader,max_epochs=25,kernel_height=kernel_height,kernel_width=kernel_width)
          
          print('loss=',int(loss))
          if(True):
            if(create_model_directory(user) != "-1"):
                model_name= 'model_'+annotation+'.pth'
                torch.save(best_model.state_dict(),os.path.join(root_directory,'model-building','user-models',user,model_name))
